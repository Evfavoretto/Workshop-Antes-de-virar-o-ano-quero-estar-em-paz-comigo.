...
<style>
  /* tenta esconder o cabeçalho do repositório (GitHub Pages) */
  ...
  :root{
    ...
    /* bleed heights: agora o topo (workshop) está dobrado */
    --bleed-height:140px; /* dobro do anterior, ajustável */
    --section-bleed-height:56px; /* outras faixas padrão */
  }
  ...
  *{box-sizing:border-box;margin:0;padding:0}
  ...
  /* ---------- FAIXA WORKSHOP (agora NÃO fixa; dobra a altura) ---------- */
  .workshop-bleed{
    position:relative; /* rolável com a página */
    ...
    box-shadow:0 10px 28px rgba(4,31,58,0.12);
    z-index:2;
    margin-bottom:12px;
  }
  .workshop-bleed h2{font-size:clamp(20px,5vw,44px);margin:0;letter-spacing:.02em}

  /* espaçador para empurrar conteúdo levemente (quando leitura rápida) */
  .top-spacer{height:8px}

  /* HERO (card branco maior) */
  .hero{background:#fff;border-radius:14px;padding:36px;margin:18px auto;box-shadow:0 30px 80px rgba(4,31,58,0.06);max-width:1150px;text-align:center}
  .hero h1{font-family:'Playfair Display',serif;font-size:clamp(28px,6vw,52px);color:var(--blue-deep);line-height:1.02;margin-bottom:12px}
  ...
  /* ---------- SECTIONS: flag full-bleed com título branco centralizado sobre ela ---------- */
  ...
  /* conteúdo principal localizado abaixo da faixa (visível) */
  .content-wide{max-width:1150px;margin:28px auto;padding:0 18px}

  /* cards das três noites */
  ...
  /* para quem / não é */
  .who-grid{display:grid;gap:18px;grid-template-columns:1fr 1fr}
  ...
  /* floating controls: tamanho fixo, não escalam */
  .countdown-float{
    position:fixed; left:14px; width:150px; height:110px; background:linear-gradient(90deg,var(--blue-deep),var(--blue-light));
    color:#fff;border-radius:14px;padding:12px;box-shadow:0 20px 50px rgba(4,31,58,0.22);z-index:2000;display:flex;flex-direction:column;align-items:center;justify-content:center;font-weight:800;text-align:center
  }
  .whatsapp-float{position:fixed; right:14px; width:160px;height:56px;background:#25D366;color:#fff;border-radius:999px;box-shadow:0 20px 46px rgba(0,0,0,0.18);z-index:2000;display:flex;align-items:center;justify-content:center;font-weight:800;cursor:pointer}
  ...
  @media(max-width:980px){
    :root{ --section-bleed-height:40px } /* make section flags a bit smaller on mobile */
    .workshop-bleed{height:calc(var(--bleed-height) * 0.75)} /* keep top headline big but slightly reduced on small screens */
    .hero{padding:20px}
  ...
  @media(max-width:420px){
    ...
    .workshop-bleed{height:calc(var(--bleed-height) * 0.66)}
  }
</style>
</head>
<body>

  <!-- FAIXA WORKSHOP full-bleed (agora rolável, altura dobrada) -->
  ...
  <div class="wrap">
    ...
    <!-- FAQ (todos os itens restaurados) -->
...
<script>
  ...
  // floats: seguem rolagem mas mantêm tamanho (controlado)
  (function(){
    const cd = document.getElementById('countdown'); const wbtn = document.getElementById('whatsBtn');
    const startCd = 140, startBtn = 220, speed = 0.14;
    function onScroll(){
