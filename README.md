...
<style>
  :root{
    --page-bg: #081427;       /* fundo escuro da página (novo) */
    --panel-bg: #fff;         /* cards brancos */
    --blue-deep: #0F4C81;     /* tons azuis usados nas faixas */
    --blue-strong:#0b3260;
    --muted: #6B7380;
    --maxw:1150px;
    --work-h:140px;           /* altura original da faixa (desktop) */
    --work-h-small:72px;      /* altura reduzida (quando rolado) */
  }
  ...
  *{box-sizing:border-box;margin:0;padding:0}
  ...
  /* Esconder cabeçalho GitHub Pages caso apareça */
  ...
  .workshop-bleed{
    ...
    transition:height .28s ease, box-shadow .28s ease, padding .28s ease;
  }
  .workshop-bleed.shrink{ height:var(--work-h-small); box-shadow:0 6px 18px rgba(0,0,0,0.35); }
  .workshop-bleed h1{
    font-family:'Playfair Display',serif;color:#fff;font-size:clamp(18px,3.2vw,40px);letter-spacing:.02em;margin:0;
    transition:transform .28s ease;
  }
  .workshop-bleed.shrink h1{ transform:translateY(-2px) scale(.92); }

  /* espaço para empurrar conteúdo para baixo (igual à altura inicial da faixa) */
  .top-spacer{height:calc(var(--work-h) + 18px); width:100%}

  .container{max-width:var(--maxw);margin:0 auto;padding:18px}

  /* HERO (card branco sobre fundo escuro) */
  .hero{
    background:var(--panel-bg);border-radius:14px;padding:46px 34px;margin:12px auto 36px;
    box-shadow:0 30px 80px rgba(0,0,0,0.35);color:var(--blue-strong);
    max-width:1100px;text-align:center;
  }
  .hero h2{font-family:'Playfair Display',serif;font-size:clamp(28px,5.8vw,56px);margin:0 0 12px;color:var(--blue-strong)}
  .hero p{color:var(--muted);max-width:860px;margin:0 auto 2px;line-height:1.6}

  /* FAIXA AS TRÊS NOITES (full-bleed azul) */
  #noites{
    width:100vw; margin-left:calc(-50vw + 50%); padding:84px 18px 64px; background:linear-gradient(180deg,var(--blue-deep),#0b64a0);
  ...
  .noites-grid .card{background:var(--panel-bg);color:#213244;padding:22px;border-radius:12px;box-shadow:0 12px 34px rgba(0,0,0,0.28);text-align:left}
  .noites-grid h4{color:var(--blue-strong);font-family:'Playfair Display',serif;margin-bottom:8px}

  /* OBJETIVOS — título em faixa azul (menor) */
  #objetivos{padding:52px 18px;background:transparent}
  #objetivos .title{display:inline-block;background:var(--blue-strong);color:#fff;padding:12px 22px;border-radius:6px;font-family:'Playfair Display',serif;font-size:22px;margin-bottom:20px}
  ...
  .objetivos-grid .item{background:var(--panel-bg);padding:18px;border-radius:12px;box-shadow:0 12px 34px rgba(0,0,0,0.16)}
  ...
  .who-box{background:var(--panel-bg);border-radius:12px;padding:16px;box-shadow:0 12px 34px rgba(0,0,0,0.12)}
  ...
  .mentor-photo{width:260px;height:260px;border-radius:12px;overflow:hidden;flex:0 0 260px;box-shadow:0 20px 50px rgba(0,0,0,0.22)}
  ...
  .mentor-bio p{color:#dbe8fb}

  /* FAQ */
  #faq{padding:46px 18px}
  .faq-wrap{max-width:var(--maxw);margin:0 auto}
  .faq-item{background:var(--panel-bg);border-radius:12px;padding:16px;box-shadow:0 12px 30px rgba(0,0,0,0.12);margin-bottom:12px;cursor:pointer}
  ...
  /* floats: contagem e whats */
  .countdown-float{
    position:fixed; left:12px; width:150px; height:110px;
    background:linear-gradient(90deg,#0b64a0,#5fb0e8); color:#fff; border-radius:14px;
    padding:10px; box-shadow:0 24px 60px rgba(0,0,0,0.4); z-index:1500; display:flex;flex-direction:column;align-items:center;justify-content:center;text-align:center;font-weight:800
  }
  ...
  @media(max-width:980px){
    .workshop-bleed{height:calc(var(--work-h) * 0.72)}
    .workshop-bleed.shrink{height:var(--work-h-small)}
  ...
  @media(max-width:420px){
    .workshop-bleed{height:calc(var(--work-h) * 0.56)}
...
<body>
  ...
  <!-- AS TRÊS NOITES (faixa azul larga) -->
  ...
  <div class="container">
    ...
    <section id="mentor" aria-labelledby="mentor-title" style="margin-top:28px">
      ...
      <div class="mentor-wrap" style="margin-top:8px">
        <div class="mentor-photo" aria-hidden="false"><img src="Mentor.jpeg" alt="Foto de Evandro Favoretto — mentor do workshop"></div>
...
<script>
  ...
  // Countdown to 02 Dec 2025 20:00 (local)
  ...
  // float positions follow scroll (limited)
  (function(){
  ...
  // FAIXA WORKSHOP — encolhe quando scrolar (acompanha a página)
  (function(){
    const bleed = document.getElementById('workBleed');
    let lastScroll = 0;
    function checkScroll(){
      const s = window.scrollY || window.pageYOffset;
      // se rolou mais que 40px, aplica shrink; caso contrário volta ao normal
      if(s > 40) bleed.classList.add('shrink'); else bleed.classList.remove('shrink');
      lastScroll = s;
    }
