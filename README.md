...
<!--
  INSTRUÇÕES RÁPIDAS:
  - Para mudar cores: edite --blue-dark / --blue-deep / --blue-light no :root
  - Para ajustar altura da faixa WORKSHOP: edite --bleed-height (agora está maior)
  - Mentor.jpeg deve estar na mesma pasta do index.html
  - Para remover o título do repositório do GitHub Pages por completo, adicione um arquivo `_config.yml` com:
      theme: null
      title: ""
      description: ""
-->
...
<style>
  /* tenta esconder cabeçalho do repositório (quando injetado) */
  ...
  :root{
    ...
    /* ALTURAS (ajuste conforme desejar) */
    --bleed-height: 140px;         /* FAIXA WORKSHOP — maior (dobra) */
    --section-bleed-height: 56px;  /* faixas das seções */
    --float-width: 150px;
    --float-height: 110px;
  }
  ...
  *{box-sizing:border-box;margin:0;padding:0}
  ...
  /* ========== FAIXA WORKSHOP (FULL-BLEED, FIXA NO TOPO) ========== */
  .workshop-bleed{
    position:fixed;
  ...
  /* espaço para empurrar conteúdo principal para baixo (altura da faixa + margem) */
  .top-spacer{height:calc(var(--bleed-height) + 12px); width:100%}

  /* HERO (QUADRO BRANCO) */
  .hero{
    background:#fff;border-radius:14px;padding:36px;margin:18px auto 34px;
  ...
  .underline{
    width:220px;height:9px;border-radius:999px;background:linear-gradient(90deg,var(--blue-deep),var(--blue-light));margin:12px auto;
  }
  ...
  .section-flag{
    ...
    width:100vw; height:var(--section-bleed-height); top:0; background:var(--blue-dark); z-index:0;
  }
  .section-title{
  ...
  .card{background:#fff;border-radius:12px;padding:18px;box-shadow:0 12px 34px rgba(4,31,58,0.04);border:1px solid rgba(0,0,0,0.03)}
  ...
  .mentor h3{color:var(--blue-deep);margin-bottom:10px}
  ...
  .faq-item h4{font-family:'Playfair Display',serif;color:var(--blue-deep);margin:0 0 6px}
  ...
  /* floats: contagem e whats (fixos, seguem rolagem controlada) */
  .countdown-float{
    ...
    padding:12px; box-shadow:0 20px 50px rgba(4,31,58,0.22); z-index:1600;
    display:flex;flex-direction:column;align-items:center;justify-content:center;font-weight:800; text-align:center;
  }
  .countdown-float .time{font-size:15px}
  .whatsapp-float{
    position:fixed; right:14px; width:170px; height:56px; background:#25D366; color:#fff; border-radius:999px;
  ...
  @media(max-width:980px){
    :root{ --section-bleed-height:44px }
    .workshop-bleed{height:calc(var(--bleed-height) * 0.72)} /* slightly smaller on tablet/phone */
    .hero{padding:22px}
    ...
    .footer-bleed{font-size:14px}
  }
  @media(max-width:420px){
    .countdown-float{width:128px;height:88px}
    .whatsapp-float{width:130px;height:48px}
    .workshop-bleed{height:calc(var(--bleed-height) * 0.6)} /* mobile tighter */
  }
</style>
</head>
<body>

  <!-- FAIXA WORKSHOP full-bleed NO TOPO (FIXA) -->
  <div class="workshop-bleed" role="banner" aria-hidden="true">
    <h2>WORKSHOP</h2>
  </div>

  <!-- ESPAÇADOR para empurrar conteúdo abaixo da faixa fixa -->
  ...
  <div class="wrap" role="document">
    ...
    <!-- AS TRÊS NOITES (com faixa full-bleed atrás do título) -->
...
<script>
  ...
  // ---- Floats: follow scroll but fixed size (prevents scaling)
  (function(){
    const cd = document.getElementById('countdown');
    const wbtn = document.getElementById('whatsBtn');
    const startCd = 140, startBtn = 220, speed = 0.12;
    function onScroll(){
